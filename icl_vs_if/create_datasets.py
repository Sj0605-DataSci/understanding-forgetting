...
for lang, (ICL_TASK, IF_TASK, kshot), INSTRUCTION_TEMPLATE, PROMPT_TEMPLATE in product(LANG_LIST, TASK_LIST, INSTRUCTION_TEMPLATE_LIST, PROMPT_TEMPLATE_LIST):    
    with open(f"/kaggle/working/understanding-forgetting/icl_vs_if/sentences/random_sentences_{lang}.txt", 'r', encoding='utf-8') as f:
    ...
    # Prefixes and instructions for each language
    prefixes = {
        'input': {
            'en': ('Input: ', 'Output: '),
            'fr': ('Saisir: ', 'Sortir: '),
    ...
    instr = {
        'noinstr': {
            'repeat': {lang: '' for lang in LANG_LIST},
            'capslock': {lang: '' for lang in LANG_LIST},
        },
        'instr': {
            'repeat': {
                'en': 'Repeat the input.\n\n',
    ...
    df.to_csv(f"/kaggle/working/understanding-forgetting/icl_vs_if/in_csvs/{ICL_TASK}-{IF_TASK}-{INSTRUCTION_TEMPLATE}-{PROMPT_TEMPLATE}-{lang}-{kshot}shot.csv")
